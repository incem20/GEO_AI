...
AfetRota is a sophisticated routing agent designed to provide the safest possible evacuation paths to assembly points in Istanbul following a major seismic event. Unlike standard navigation tools that prioritize the shortest path, AfetRota dynamically evaluates seismic risk data to steer users away from high-vulnerability zones and potential debris bottlenecks.
...
1.  **Install dependencies:**
    ...
    ```
2.  **Configuration:** Create a `.env` file in the root directory and add the following:
    ```env
    USE_LIVE_OSM=true
    LLM_MODEL=claude-sonnet-4-6
    ```
3.  **Data Initialization:** Run the processing scripts to prepare your local database:
    ```bash
...
*Other districts will also provide results by downloading live maps from OSMnx; however, risk-mapping is non-existent there because the IBB live dataset is currently unavailable.*